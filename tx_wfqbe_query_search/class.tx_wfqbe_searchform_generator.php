...
class tx_wfqbe_searchform_generator{
	...
	// datepicker toegevoegd EW 18-1-2016 
	var $types = array ('input', 'radio', 'select', 'check', 'calendar', 'datepicker');
	...
	* @return	[string]	$content: stringa che contiene l'html del form	
	...
	function renderField($key, $value, $h)	{
		...
		if ($numForm > 0)
			$content.= ' <a href="#" title="Move up ('.($numForm).')" onclick="document.getElementById(\'wfqbe_up\').value='.($numForm).'; updateForm()"><img src="'.\TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('TYPO3_SITE_URL').'typo3/sysext/t3skin/icons/gfx/button_up.gif" /></a>';
		if ($numForm < sizeof($this->blocks['fields'])-2)
			$content.= ' <a href="#" title="Move down ('.($numForm).')" onclick="document.getElementById(\'wfqbe_down\').value='.($numForm).'; updateForm()"><img src="'.\TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('TYPO3_SITE_URL').'typo3/sysext/t3skin/icons/gfx/button_down.gif" /></a>';
