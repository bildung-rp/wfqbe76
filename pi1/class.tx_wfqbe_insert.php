	        	...
	        	elseif ($results['id']=='-2' && $this->conf['ff_data']['deletedPage']!='')	{
					...
					$params['parameter'] = $this->conf['ff_data']['deletedPage'];
	        			...
	        			if ($this->conf['ff_data']['redirectQuery']!='') {
							if (is_array($results['insert_data'])) foreach ($results['insert_data'] as $key => $value)	{
		        				$mA['###FIELD_'.$key.'###'] = $value;
				...
				foreach ($blocks['fields'] as $key => $config)	{
					if ($config['form']['when'][$this->mode]==1)	{
						...
						elseif ($config['type']=='PHP function')	{
							...
							continue;
						}	elseif ($config['type']=='textarea' && $config['form']['rte']==1 && \TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded('rtehtmlarea'))	{
							if($this->RTEObj->isAvailable()) {
